...
public class Student {
    ...
    public Student(String name, String seatNo, Semester semesterForSemNo) {
        ...
        if (index >= 0 && index < TOTAL_SEMESTERS) {
            ...
            this.semCount = index + 1; // This is the fix!
        }else {
    ...
    public String toString() {
        ...
        for (int i = 0; i < TOTAL_SEMESTERS; i++) {
            ...
            if (sem != null && sem.getCourseCount() > 0) {
                sb.append("\nSemester ").append(sem.getSemNumber()).append(":\n");

                // Print courses numbered
                for (int j = 0; j < sem.getCourseCount(); j++) {
                    Course c = sem.getCourse(j);
                    sb.append("  ").append(j + 1).append(". ")
                            .append(c.getCourseName().trim())
